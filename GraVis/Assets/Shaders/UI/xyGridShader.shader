    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 calcField(float3 fragCoord3D, float scale, float distance)
            {
                ...
                float2 coord = fragCoord3D.xy * scale; // use the scale variable to set the distance between the lines
                float2 derivative = fwidth(coord);
                float2 grid = abs(frac(coord - 0.5) - 0.5) / derivative;
                float lin = min(grid.x, grid.y);
                float minimumz = min(derivative.y, 1);
                float minimumx = min(derivative.x, 1);
                float4 color = float4(0.1, 0.1, 0.1, 1 - min(lin, 1.0));
                color.a *= 0.3f;
                // y axis
                if (fragCoord3D.x > -0.02 * minimumx && fragCoord3D.x < 0.01 * minimumx)
                    color.y = 1.0;
                // x axis
                if (fragCoord3D.y > -0.02 * minimumz && fragCoord3D.y < 0.01 * minimumz)
                    color.x = 1.0;
                color.a -= degration;
