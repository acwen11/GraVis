    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 calcField(float3 fragCoord3D, float scale, float distance)
            {
                ...
                float2 coord = fragCoord3D.yz * scale; // use the scale variable to set the distance between the lines
                ...
                color.a *= 0.6f;
                // y axis
                if (fragCoord3D.z > -0.1 * minimumx && fragCoord3D.z < 0.1 * minimumx)
                    color.y = 1.0;
                // z axis
                if (fragCoord3D.y > -0.1 * minimumz && fragCoord3D.y < 0.1 * minimumz)
                    color.z = 1.0;
                color.a -= degration;
